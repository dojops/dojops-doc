...
3. **Auto-install** — If the external binary isn't installed, DojOps auto-installs it into the sandboxed toolchain (`~/.dojops/toolchain/`) via the `OnBinaryMissing` callback before running verification. This replaces the previous behavior of silently skipping verification when a binary was missing
...
### OnBinaryMissing Callback

The `SafeExecutor` accepts an optional `OnBinaryMissing` callback that is invoked when a verification command references a binary not found on `$PATH`. The default implementation auto-installs the binary via the `dojops toolchain install` mechanism into `~/.dojops/toolchain/`. This ensures verification is never silently skipped due to a missing tool.

### Self-Repair Loop (CriticAgent)

When verification fails, the executor can invoke a `CriticAgent` self-repair loop. The CriticAgent receives the verification errors and the generated output, then asks the LLM to fix the issues. The corrected output is re-verified, and the loop repeats until verification passes or a maximum retry count is reached. This enables automatic recovery from common generation errors (e.g., invalid YAML syntax, missing required fields) without user intervention.
