...
DojOps includes 35 built-in DevOps skills covering CI/CD, infrastructure-as-code, containers, monitoring, and system services. All skills follow a consistent pattern built on the `BaseSkill<T>` abstract class. A custom skill system lets you extend DojOps with your own skills via declarative `.dops` files.
...
All 35 built-in skills are defined as `.dops v2` skill files in `packages/runtime/skills/`. Each skill is processed by `DopsRuntime`, which compiles prompts, calls the LLM, and writes raw file content directly (no JSON→serialize step).
