...
- A configured LLM provider (any of the 6 supported providers)
...
The `dojops metrics tokens` command pulls all recorded usage from your `.dojops/` history and presents a summary.

```bash
dojops metrics tokens
```
...
```bash
dojops metrics tokens --by-skill
```
...
```bash
dojops metrics tokens --output json
```
...
```bash
dojops metrics tokens --output json > usage-$(date +%Y-%m-%d).json
```
...
**`dojops metrics tokens` shows zero usage**
...
Token costs fall into two categories: tokens you control through model selection, and tokens you can eliminate through compression. `dojops metrics tokens` gives you the breakdown to know which category matters most for your workflow. Smart compression is automatic — it activates on `debug-ci` and `scan --fix` without any configuration — and it routinely cuts 70–95% of tokens on noisy CI logs. Requesting JSON from scanners adds another 20–40% reduction on top of that. Combined, a typical `debug-ci` run on a large log file costs less than a fraction of a cent instead of several cents.
