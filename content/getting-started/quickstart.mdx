...
See [Installation](/getting-started/installation) for upgrade/uninstall instructions and troubleshooting.
...
See [Configuration](/getting-started/configuration) for the full list of providers, env vars, and precedence rules.
...
> **Auto-install:** DojOps automatically installs missing verification tools (e.g., `terraform`, `hadolint`, `actionlint`) into a sandboxed toolchain directory (`~/.dojops/toolchain/`) on first use. You don't need to install them manually.
...
See [Security Scanning](/components/security-scanning) for details on all 10 scanners.
...
See [Web Dashboard](/usage/dashboard) for the full guide.

---

## Extending with Custom Tools

DojOps supports custom tools via a declarative `.dops` module system. Scaffold a module with `dojops modules init`, which creates a single `.dops` file combining metadata, schemas, and prompts:

```bash
# Scaffold a new .dops module
dojops modules init my-tool

# List discovered custom modules
dojops modules list
...
See [DevOps Tools — Custom Tool System](/components/tools#custom-tool-system) for the full guide.
...
- **[CLI Reference](/usage/cli)** — Full command documentation (including `dojops check`)
- **[API Reference](/usage/api)** — REST API for programmatic access
- **[Architecture](/architecture/overview)** — System design overview
- **[Configuration](/getting-started/configuration)** — Advanced provider and profile setup
