...
| `DOJOPS_HUB_URL`                 | DojOps Hub API base URL                                                        | `https://hub.dojops.ai`  |
| `DOJOPS_HUB_TOKEN`               | API token for hub publishing ([generate one](/usage/cli#hub-publishing-setup)) | --                       |
...
When you add a provider with `dojops provider add`, DojOps encrypts the token using a passphrase (prompted interactively) or the `DOJOPS_VAULT_KEY` environment variable. Encrypted tokens are stored in `~/.dojops/vault.json`.

```bash
# For CI/CD, set the vault passphrase via environment variable
export DOJOPS_VAULT_KEY="your-passphrase"

# Check vault status
dojops config vault-status
```
...
dojops config apply staging-config.yaml  # Import from YAML/JSON
```
...
MCP (Model Context Protocol) servers are configured separately from LLM providers. See [MCP](/components/mcp) for full documentation.

```bash
# Add an MCP server interactively
dojops mcp add

# Or create .dojops/mcp.json manually
```

MCP servers extend the autonomous agent (`dojops auto`) with external tools from databases, cloud APIs, and other services.
