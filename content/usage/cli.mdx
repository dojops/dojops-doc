...
Chat supports slash commands: `/exit`, `/agent <name>`, `/voice`, `/plan <goal>`, `/apply`, `/auto <prompt>`, `/scan`, `/history`, `/clear`, `/save`.
...
# Scaffold a new v2 .dops skill (uses AI when provider is configured)
dojops skills init my-skill

# Scaffold with legacy v1 format
dojops skills init my-skill --legacy
