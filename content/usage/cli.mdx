...
The CLI sends the token as a `Bearer` header. Tokens can be managed (created, viewed, revoked) from the Hub Settings page at `/settings/tokens`. See [DevOps Tools — Hub Integration](/components/tools#hub-integration) for the full publish/install flow.
