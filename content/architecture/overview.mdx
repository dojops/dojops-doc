...
DojOps is a modular DevOps agent system with 13 built-in modules, a custom module system, 17 specialist agents, sandboxed execution, approval workflows, and hash-chained audit trails. Every LLM output is schema-validated and policy-checked before anything gets written.
...
@dojops/runtime        13 built-in DevOps modules as .dops files (DopsRuntime)
@dojops/scanner        10 security scanners + remediation engine
@dojops/session        Chat session management + memory + context injection
@dojops/context        Context7 documentation augmentation for modules
...
See [DevOps Modules](/components/modules) for the module pattern.

### 4b. DOPS Runtime (`@dojops/runtime`)

The DOPS runtime processes `.dops` module files — a declarative format combining YAML frontmatter with markdown prompt sections. Raw content generation with Context7 integration.
...
- `DopsRuntime` — Runtime class for `.dops` modules
...
- `DocProvider` interface — Enables Context7 documentation augmentation for modules
...
### 5. DevOps Modules (`@dojops/runtime`)

13 built-in modules covering CI/CD, IaC, containers, monitoring, and system services. All 13 are `.dops` modules in `packages/runtime/modules/`, processed by `DopsRuntime` — generating raw file content directly via LLM with Context7 documentation augmentation. All modules support updating existing configs via auto-detection, `existingContent` input, and `.bak` backup before overwrite. All file writes use `atomicWriteFileSync()` for crash safety. Every `execute()` returns `filesWritten`/`filesModified` for rollback tracking.

See [DevOps Modules](/components/modules) for the full module list.
...
- **`.dops` module discovery** — Discovers `.dops` modules from `.dojops/modules/` (project) and `~/.dojops/modules/` (global)
...
Express-based API with dependency injection via `createApp(deps)`. Uses `@dojops/module-registry` to load all built-in + custom modules. 20 endpoints exposing all capabilities over HTTP with API v1 versioning (`/api/v1/` prefix with backward-compatible `/api/` alias, `X-API-Version: 1` header on v1 routes). Vanilla web dashboard with 5 tabs (Overview, Security, Audit, Agents, History). Health endpoint reports `customModuleCount`. Per-route rate limiting and token budget tracking via `TokenTracker`.
...
6. **Extensibility** — New modules follow the `BaseModule<T>` pattern. New agents are registered in the specialist list.
