...
All 13 built-in modules and custom modules also use `atomicWriteFileSync()` from `@dojops/sdk` for direct file writes outside the sandbox.
...
Custom modules execute through the same `SafeExecutor` pipeline as built-in modules (inheriting `maxFileSize`, `timeoutMs`, DevOps write allowlist, and per-file audit logging), plus three additional security controls:

### Verification Command Whitelist

The `verification.command` field in module manifests can only invoke one of 33 whitelisted binaries:

```
terraform, kubectl, helm, ansible-lint, ansible-playbook, docker,
hadolint, yamllint, jsonlint, shellcheck, tflint, kubeval,
conftest, checkov, trivy, kube-score, polaris, nginx,
promtool, systemd-analyze, make, actionlint, caddy, haproxy,
nomad, podman, fluentd, opa, vault, circleci, npx, tsc, cfn-lint
```
...
This ensures custom modules cannot execute shell commands unless they explicitly declare `child_process: "required"` AND the command is whitelisted.

### Path Traversal Prevention

File paths in `files[].path` and `detector.path` are validated at schema level — any segment containing `..` is rejected. This prevents custom modules from writing to or detecting files outside the project directory (e.g., `../../etc/passwd`).

### Module Hash Integrity

Each custom module has a SHA-256 hash computed from its `.dops` file content. This hash is pinned into plans at creation time and validated on `--resume` and `--replay` to detect module modifications between plan creation and execution.
...
3. Validating `systemPromptHash` for custom module tasks to detect prompt drift
...
This eliminates plaintext API token storage, which is a common credential exposure vector.
