...
The `verification.command` field in tool manifests can only invoke one of 33 whitelisted binaries:
...
Each custom tool has a SHA-256 hash computed from its `tool.yaml` content. This hash is pinned into plans at creation time and validated on `--resume` and `--replay` to detect tool modifications between plan creation and execution. See [Tool Specification v1](/components/tool-spec-v1) for the full security model.
