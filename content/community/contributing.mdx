...
Contributions to DojOps are welcome. This guide covers development setup, coding standards, testing, and how to add new modules and agents.
...
packages/
  ...
  module-registry/  Module registry + custom module system (built-in + custom module discovery)
...
All 13 built-in modules are `.dops` files in `packages/runtime/modules/`. To add a new built-in module, create a `.dops` file following the format. See [DevOps Modules](/components/modules) for the full pattern.

### Step-by-Step

1. **Create the `.dops` module file:** `packages/runtime/modules/my-tool.dops`
...
2. **Register the module** in `packages/runtime/src/built-in-modules.ts` — add the filename to the module list.
...
4. **Write tests:** Create `packages/runtime/src/__tests__/modules/my-tool.test.ts` with mocked LLM provider — test prompt compilation, update mode, and verification.

---

## Creating a Custom Module

For modules that don't need to be built into the core, use the custom module system instead. Custom modules are declarative — no TypeScript code required.

### Step-by-Step

1. **Scaffold** a custom module:

   ```bash
   dojops modules init my-tool
   ```

   This creates `.dojops/modules/my-tool.dops` — a single `.dops` file combining YAML frontmatter with markdown prompt sections.
...
3. **Validate** the module:
   ...
   ```bash
   dojops modules validate my-tool
   ```
...
Custom modules are automatically discovered and available to the Planner, Executor, and API. See [DevOps Modules — Custom Module System](/components/modules#custom-module-system) for the full `.dops` format.
...
- [ ] New modules follow the `BaseModule<T>` pattern
