...
Dependency flow: `cli -> api -> tool-registry -> runtime -> core -> sdk`
...
All 13 built-in tools are `.dops v2` module files in `packages/runtime/modules/`. To add a new built-in tool, create a `.dops` file following the v2 format. See [DevOps Tools](/components/tools) for the full pattern.

### Step-by-Step

1. **Create the `.dops` module file:** `packages/runtime/modules/my-tool.dops`

   A `.dops v2` file has YAML frontmatter followed by markdown prompt sections:
...
Custom tools are automatically discovered and available to the Planner, Executor, and API. See [DevOps Tools — Custom Tool System](/components/tools#custom-tool-system) for the full `.dops` format.

> **Note:** Use `dojops modules init my-tool --legacy` to scaffold the old `tool.yaml` + `input.schema.json` format instead.
